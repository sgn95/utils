<?php
    ...
    class JianFan {
         ...
         * @return 繁体转简体
         */
        public function traditionToSimple ($sContent) {
            $simpleCN = '';
            $iContent = mb_strlen( $sContent, 'UTF-8' );
            for ($i = 0; $i < $iContent; $i++) {
                $str = mb_substr( $sContent, $i, 1, 'UTF-8' );
                $match = mb_strpos( JianFan::$td, $str, null, 'UTF-8' );
                $simpleCN .= ( $match !== false ) ? mb_substr( JianFan::$sd, $match, 1, 'UTF-8' ) : $str;
            }
         ...
         * @return 简体转繁体
         */
        public function simpleToTradition ($sContent) {
            $traditionalCN = '';
            $iContent = mb_strlen( $sContent, 'UTF-8' );
            for ($i = 0; $i < $iContent; $i++) {
                $str = mb_substr( $sContent, $i, 1, 'UTF-8' );
                $match = mb_strpos( JianFan::$sd, $str, null, 'UTF-8' );
                $traditionalCN .= ( $match !== false ) ? mb_substr( JianFan::$td, $match, 1, 'UTF-8' ) : $str;
            }
